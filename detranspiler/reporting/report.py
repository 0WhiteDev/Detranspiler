...
def _render_source_provenance(data: Any) -> str:
    if not isinstance(data, dict):
        return '<p class="empty">Source provenance unavailable.</p>'
    coverage = data.get('native_mapping_coverage')
    return kv_table([('Status', escape_html(str(data.get('status') or 'n/a'))), ('Files mapped', data.get('files_total', 0)), ('Lines covered', data.get('lines_total', 0)), ('Methods mapped', data.get('methods_total', 0)), ('Bodies with evidence', data.get('methods_with_body_evidence', 0)), ('Native-generated bodies', data.get('methods_with_generated_body_evidence', 0)), ('Native links', f"{data.get('native_methods_linked', 0)} / {data.get('native_methods_total', 0)}"), ('Mapping coverage', f'{round(float(coverage) * 100)}%' if isinstance(coverage, (int, float)) else 'n/a'), ('Evidence records', data.get('evidence_total', 0)), ('Sidecar', file_link(data.get('output_path')))])
...
def write_html_report(*, job: Dict[str, Any], out_path: Path) -> Dict[str, Any]:
    ...
    source_provenance = _get(job, 'analysis.source_provenance', {})
    ...
    sidebar = side_nav([('Overview', '#overview'), ('Recovery', '#recovery'), ('Methods', '#methods'), ('Provenance', '#provenance'), ('Artifacts', '#artifacts'), ('Analysis', '#analysis'), ('Strings', '#strings'), ('Call graph', '#callgraph'), ('Patterns', '#patterns'), ('JNI', '#jni'), ('Deobfuscation', '#deobfuscation'), ('Anti-analysis', '#anti'), ('Java validation', '#java-validation'), ('Confidence', '#confidence'), ('JNIC', '#jnic'), ('job.json', '#job')])
