...
def launch_gui(*, width: Optional[int]=None, height: Optional[int]=None) -> int:
    ...
    if not index_html.is_file():
        ...
        return 1
    window = webview.create_window(title=f'Detranspiler {api.get_version()}', url=str(index_html), js_api=api, width=int(width or settings.get('window_width') or 1280), height=int(height or settings.get('window_height') or 860), min_size=(960, 640), background_color='#0d1117')
